# js2
# About Js
> JavaScript - это интерпретируемый язык программирования, который часто используется в веб-разработке. Он был создан в 1995 году Бренданом Эйхом в компании Netscape и был исходно изначально назван Mocha, а затем LiveScript перед тем, как получить свое текущее имя.

JavaScript позволяет создавать динамические веб-страницы, а также приложения, которые могут быть использованы в браузере и вне его. Он может добавлять интерактивность на странице, а это означает, что пользователи могут взаимодействовать со страницей и изменять ее содержимое без перезагрузки. JavaScript широко используется для создания пользовательских Интерфейсов и взаимодействия с пользователем. Он позволяет создавать формы, кнопки, меню, всплывающие окна и другие элементы интерфейса, которые могут быть использованы для обработки пользовательского ввода и управления поведением страницы. JavaScript также позволяет работать с браузерным API и манипулировать содержимым документа, изменяя CSS свойства и добавляя или удаляя HTML элементы.

![](data:image/png;base64,)

# Основы js
> Основные концепции JavaScript включают в себя переменные, операторы, условные выражения, циклы, функции и объекты.

```js
for();let;var;const;if;else if;function// and others
```

# Variables

> Переменные - это контейнеры для хранения значений, которые могут быть числами, строками или boolean

```js
let a = 5
console.log(a) //5
```

# Оператоы
> Операторы - это символы, которые выполняют операции над переменными и значениями, например, сложение (+), вычитание (-), умножение (*) и деление (/).

```
+ = - / * && || ! < > <= >= == === 
```

# Условный опеатор
> Условные выражения - это конструкции, которые позволяют выполнить различный код в зависимости от значения переменной или выражения. Например, оператор if проверяет условие и выполняет код только в том случае, если условие истинно. 
```js
let a = 5
if(a==2){
    console.log('2')
}else if(a==5){
    console.log('5')
}else{
    console.log(error)
}

```
# loops
> Циклы - это конструкции, которые позволяют повторять код несколько раз. Например, цикл for выполняет определенный код заданное количество раз.

```js
for(let i = 0;i<4;i++){
    console.log(i)//0 1 2 3 
}
```

# Function
> Функции - это блоки кода, которые могут быть вызваны из других частей программы. Функции могут принимать параметры и возвращать значения. 

```js
function name(a,b){
    return a+b
}
console.log(name(4,5))//9
```

# end
> JavaScript также поддерживает работу с событиями, которые могут происходить на веб-странице, такими как щелчки мыши и нажатия клавиш, и манипулировать содержимым HTML-элементов на странице. JavaScript является одним из самых популярных языков программирования в мире и широко используется веб-разработчиками для создания интерактивных веб-сайтов и приложений.

